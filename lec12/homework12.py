...
def voiced_excitation(duration, F0, Fs):
    ...
    # Initialize the excitation signal with zeros
    excitation = np.zeros(duration)
    
    # Set indices that are multiples of T0 to -1
    excitation[::T0] = -1
...
def resonator(x, F, BW, Fs):
    ...
    '''
    # Calculate coefficients based on frequency and bandwidth
    ...
    # Difference equation: y[n] = A*x[n] + B*y[n-1] + C*y[n-2]
...
def synthesize_vowel(duration, F0, F1, F2, F3, F4, BW1, BW2, BW3, BW4, Fs):
    ...
    @returns:
    y (np.ndarray(duration)) - synthesized vowel
    '''
    # Step 1: Generate the voiced excitation
    excitation = voiced_excitation(duration, F0, Fs)
    
    # Step 2: Pass through 4 resonators in series (Formants 1 to 4)
    y1 = resonator(excitation, F1, BW1, Fs)
    y2 = resonator(y1, F2, BW2, Fs)
    y3 = resonator(y2, F3, BW3, Fs)
    y = resonator(y3, F4, BW4, Fs)
